...
#%%
"""" output difference """
G2_difference = G2_new[1:]-G2_new[:-1]
G2_base = G2_new[:-1]
...
def DNN_model2(timestep):
    ...
    # final_output = LSTM(6,stateful=False,return_sequences=True,kernel_regularizer=regularizers.L2(l2=1e-2))(final_output)
    ...
    # final_output = tf.keras.layers.BatchNormalization()(final_output) 
...
preprocessing_module=multi_input_output(G2_new[1:,:],input_timestep=timestep,output_timestep=timestep)
G_multi_input_origin=preprocessing_module.generate_input()
G_multi_output_origin=preprocessing_module.generate_output()

preprocessing_module=multi_input_output(G2_difference,input_timestep=timestep,output_timestep=timestep)
...
date=pd.DataFrame(G2.iloc[timestep-1+forecast_timestep[0]:-(timestep-forecast_timestep[0])].index)
...
save_path=r"model\2nd_layer\hbv-ann.hdf5"
...
for i in range(0,3):
    writer = pd.ExcelWriter(r"result\2nd_layer\hbvann-forecast(t+%s).xlsx"%(i+1),engine='xlsxwriter')
...
for i in range(0,3):
    writer = pd.ExcelWriter(r"result\2nd_layer\hbvann-test(t+%s).xlsx"%(i+1),engine='xlsxwriter')
    pd.DataFrame(performance.iloc[i,:]).to_excel(writer,sheet_name="all_performance")
    pd.DataFrame(pred_test_new[i]).to_excel(writer,sheet_name="test_eachstation")
    pd.DataFrame(test_new_rmse[i]).to_excel(writer,sheet_name="test_rmse_eachstation")                
    pd.DataFrame(test_new_R2[i]).to_excel(writer,sheet_name="test_R2_eachstation")
...
split_index=int(0.8*len(G_train_output))
g1_station_num=G2_new.shape[1]
writer = pd.ExcelWriter(r"result\2nd_layer\sorted_predict(hbv-ann).xlsx",engine='xlsxwriter')
sorted_train_new=[]
for i in range(0,3):
    train=convert_result(pred_train[:,i],g1_station_num)
    test=convert_result(pred_test[:,i],g1_station_num)
...
#%%
writer = pd.ExcelWriter(r"result\2nd_layer\sorted_observation.xlsx",engine='xlsxwriter')
# sorted_obs_new=[]
for i in range(0,3):

    # sorted_obs_new.append(G_train_output[:,i,:])
