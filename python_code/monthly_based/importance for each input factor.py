# -*- coding: utf-8 -*-
"""
Created on Mon Feb 27 20:20:53 2023
...
def DLSTM_model(timestep):
    ...
    inputs_ = Input(batch_input_shape=(1,timestep,11))
    inputs1 = Lambda(lambda x: x[:,:,0])(inputs_)
    inputs1 = Lambda(lambda x: tf.expand_dims(x, -1))(inputs1)
    
    output = LSTM(12,stateful=True,return_sequences=True)(inputs1)
    output = LSTM(12,stateful=True,return_sequences=False)(output)
    
    inputs2 = Lambda(lambda x: x[:,:,1:])(inputs_)
    output2 = LSTM(12,stateful=True,return_sequences=True)(inputs2)
    output2 = LSTM(12,stateful=True,return_sequences=False)(output2)    
    ...
    model = Model(inputs=inputs_, outputs=final_output)
...
""" Model importance """ 
shap.initjs()
model_new=DLSTM_model(5)
all_shap_point_1=[]; all_shap_point_2=[]; all_shap_point_3=[]
all_shap_code_1=[]; all_shap_code_2=[]; all_shap_code_3=[]
for station_no in range(0,g1_station_num):
    ...
    all_input_1=np.expand_dims(np.concatenate([model_train_input_1,model_train_input_2],axis=1),axis=2)
    all_input_2=np.concatenate([model_train_input_3,model_train_input_4],axis=1)
    all_input=np.concatenate([all_input_1,all_input_2],axis=2)
    ...
    for i in range(4,len(model_new.layers)):
        model_new.layers[i].set_weights(model.layers[i+3].get_weights())
    explainer = shap.GradientExplainer(model_new, all_input,batch_size=1)
    
    shap_values =[]
    for i in range(0,len(all_input)):
        each_input=np.expand_dims(all_input[i,:,:],axis=0)
        shap_values.append (explainer.shap_values(each_input))
    # explainer = shap.DeepExplainer(model,shap_values50)
    
    total_shap =  np.squeeze(np.array(shap_values))
    total_shap = np.squeeze(np.sum(shap_values,axis=0))
    all_shap_point_1.append(total_shap[0,:,0])
    all_shap_point_2.append(total_shap[1,:,0])
    all_shap_point_3.append(total_shap[2,:,0])

    all_shap_code_1.append(total_shap[0,:,1:])
    all_shap_code_2.append(total_shap[1,:,1:])
    all_shap_code_3.append(total_shap[2,:,1:])
    
#%%
""" Save each station result to csv """ 
for station_no in range(0,g1_station_num):
    # pd.DataFrame(all_shap_point_1[station_no]).to_csv(r"result\factor importance\point\t+1\station%s.csv"%station_no)
    # pd.DataFrame(all_shap_point_2[station_no]).to_csv(r"result\factor importance\point\t+2\station%s.csv"%station_no)
    # pd.DataFrame(all_shap_point_3[station_no]).to_csv(r"result\factor importance\point\t+3\station%s.csv"%station_no)

    pd.DataFrame(all_shap_code_1[station_no]).to_csv(r"result\factor importance\code\t+1\station%s.csv"%station_no)
    pd.DataFrame(all_shap_code_2[station_no]).to_csv(r"result\factor importance\code\t+2\station%s.csv"%station_no)
    pd.DataFrame(all_shap_code_3[station_no]).to_csv(r"result\factor importance\code\t+3\station%s.csv"%station_no)    
    
concat_all_shap_point_1=pd.DataFrame(np.array(all_shap_point_1))
concat_all_shap_point_2=pd.DataFrame(np.array(all_shap_point_2))
concat_all_shap_point_3=pd.DataFrame(np.array(all_shap_point_3))

concat_all_shap_point_1.to_csv(r"result\factor importance\point\t+1\all_station.csv")
concat_all_shap_point_2.to_csv(r"result\factor importance\point\t+2\all_station.csv")
concat_all_shap_point_3.to_csv(r"result\factor importance\point\t+3\all_station.csv")

#%%
""" Save average result to csv """ 
average_shap_point=np.mean([concat_all_shap_point_1,concat_all_shap_point_2,concat_all_shap_point_3],axis=0)
average_shap_point2=np.mean(np.mean([concat_all_shap_point_1,concat_all_shap_point_2,concat_all_shap_point_3],axis=1),axis=0)
average_shap_code=np.mean(np.mean([all_shap_code_1,all_shap_code_2,all_shap_code_3],axis=1),axis=0)

writer = pd.ExcelWriter(r"result\factor importance\average_importance.xlsx",engine='xlsxwriter')
pd.DataFrame(average_shap_point).to_excel(writer,sheet_name="average_shap_point")
pd.DataFrame(average_shap_point2).to_excel(writer,sheet_name="average_shap_point2")
pd.DataFrame(average_shap_code).to_excel(writer,sheet_name="average_shap_code")
writer.save()
