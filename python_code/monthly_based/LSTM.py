# -*- coding: utf-8 -*-
"""
Created on Wed Jun  7 00:56:33 2023
...
def LSTM_model(timestep):
    ...
    final_output = Lambda(lambda x: tf.expand_dims(x, -1))(output)
    
    final_output = LSTM(18,stateful=True, activation='linear',return_sequences=True)(final_output)
    final_output = LSTM(9,stateful=True, activation='linear',return_sequences=False)(final_output)
    final_output = Dense(3, activation='linear',kernel_regularizer=regularizers.L2(l2=1e-2))(final_output)
...
G_test_input = G_multi_input_origin[test_index,1:,:] # select t-1, t timestep
...
for station_no in range(0,g1_station_num):
    ...
    model.fit([model_train_input_1,model_train_input_2], model_train_output_norm, epochs=100, batch_size=1,validation_split=0.2,callbacks=callback_list)
    model=load_model(save_path) 
    pred_train= model.predict([model_train_input_1,model_train_input_2], batch_size=1)*(train_output_max-train_output_min)+train_output_min
    pred_test = model.predict([model_test_input_1,model_test_input_2], batch_size=1)*(train_output_max-train_output_min)+train_output_min
...
writer = pd.ExcelWriter(r"result\1st_layer\predict(lstm).xlsx",engine='xlsxwriter')
