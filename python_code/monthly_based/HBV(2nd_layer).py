...
for i in range(0,len(G2_station_info_new)):
    ...
    all_info.append(get_specific_coordinate(G2_station_info_new.iloc[i,1],G2_station_info_new.iloc[i,2],X,Y))
...
G2_new_ = G2_new[timestep-1:-timestep]   
forecast_timestep=np.array([timestep/3,timestep*2/3,timestep]).astype(int)
G2_out1 = G2_new[timestep-1+forecast_timestep[0]:-(timestep-forecast_timestep[0])]; out_difference1=G2_out1[1:,:]-G2_out1[0:-1,:]
G2_out2 = G2_new[timestep-1+forecast_timestep[1]:-(timestep-forecast_timestep[1])]  
G2_out3 = G2_new[timestep-1+forecast_timestep[2]:]  
...
for index,optima in enumerate(optima_alltimestep):
    ...
    optima.to_csv(r"result\2nd_layer\optima_parameter(t+%s).csv"%(index+1))
...
for index in range(0,3):
    optima = pd.read_csv(r"result\2nd_layer\optima_parameter(t+%s).csv"%(index+1),index_col=0)
    location=1
...
writer = pd.ExcelWriter(r"result\2nd_layer\sorted_predict.xlsx",engine='xlsxwriter')
sorted_train_new=[]
for i in range(0,3):
...
# save result
os.chdir(path)
writer = pd.ExcelWriter(r"result\2nd_layer\simulate-shuffle(train).xlsx",engine='xlsxwriter')
...
writer = pd.ExcelWriter(r"result\2nd_layer\simulate-shuffle(test).xlsx",engine='xlsxwriter')
for i,simulate_train_result in enumerate(all_simulate_train_result):
...
writer = pd.ExcelWriter(r"result\2nd_layer\simulate(test).xlsx",engine='xlsxwriter')
