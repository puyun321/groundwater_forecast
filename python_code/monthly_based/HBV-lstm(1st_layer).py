...
from keras.layers import Dense,LSTM,Conv1D,Flatten,Concatenate,Lambda,Layer,Attention
...
def DNN_model(timestep):
    ...
    final_output = LSTM(12,stateful=True,return_sequences=True)(final_output)
    final_output = LSTM(12,stateful=True,return_sequences=False)(final_output)
...
simulate_merge_train=simulate_merge(simulate_train)[1:]
...
for station_no in range(0,g1_station_num):
    ...
    save_path=r"model\1st_layer\hbv-lstm(station_%s).hdf5"%station_no
    checkpoint =ModelCheckpoint(save_path,save_best_only=True)
    callback_list=[earlystopper,checkpoint]  
    model.fit([model_train_input_1,model_train_input_2], model_train_output, epochs=100, batch_size=1,validation_split=0.1,callbacks=callback_list)
    model=load_model(save_path) 
    
    """ model forecasting """ 
    pred_train=model.predict([model_train_input_1,model_train_input_2], batch_size=1)   
    
    """real test"""
    station_no = 0
    test_index=test_index-1
    G_test_input = G_multi_input_origin[test_index,1:,station_no]
    G_test_output = G_multi_output_origin[test_index,:,station_no]
    ...
    model_test_output = G_test_output
...
#%%
writer = pd.ExcelWriter(r"result\1st_layer\hbvlstm-forecast(t+%s).xlsx"%(i+1),engine='xlsxwriter')
...
for i in range(0,3):
    writer = pd.ExcelWriter(r"result\1st_layer\hbvlstm-test(t+%s).xlsx"%(i+1),engine='xlsxwriter')
    pd.DataFrame(performance.iloc[i,:]).to_excel(writer,sheet_name="all_performance")
    pd.DataFrame(pred_test_new[i]).to_excel(writer,sheet_name="test_eachstation")
    pd.DataFrame(test_new_rmse[i]).to_excel(writer,sheet_name="test_rmse_eachstation")                
    pd.DataFrame(test_new_R2[i]).to_excel(writer,sheet_name="test_R2_eachstation")
...
writer = pd.ExcelWriter(r"result\1st_layer\sorted_predict(hbv-lstm).xlsx",engine='xlsxwriter')
sorted_train_new=[]
for i in range(0,3):
...
#%%
writer = pd.ExcelWriter(r"result\1st_layer\sorted_observation(lstm).xlsx",engine='xlsxwriter')
# sorted_obs_new=[]
for i in range(0,3):
