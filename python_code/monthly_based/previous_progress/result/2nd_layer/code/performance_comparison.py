...
#%%
#sort again
data_statistic=pd.read_excel(r"data_statistic.xlsx",sheet_name="l1")
...
def plot_figure(performance,name):
    ...
    # Horizontal Bar Plot
    plt.bar(br1, performance.iloc[:,2],color='g',width = barWidth,edgecolor ='grey',label='HBV')
    ...
    if 'R2' in name:
        ...
        plt.ylim([0, 7])       
    plt.savefig(r"result\1st_layer\performance figure\%s.jpeg"%name)
    # Show Plot
    plt.show()
