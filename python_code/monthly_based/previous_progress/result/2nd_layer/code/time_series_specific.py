...
hbv_lstm=pd.read_excel(r"result\2nd_layer\sorted_predict(hbv-ann).xlsx",sheet_name="t+1(shuffle)",index_col=0)
hbv=pd.read_excel(r"result\2nd_layer\sorted_predict.xlsx",sheet_name="t+1(shuffle)",index_col=0).iloc[1:,:]
...
station_info = np.array(G2_station_info_new)
...
path1='result\\2nd_layer\\time series figure\\%s'%start_date
...
for station in stations:
    ...
    plt.ylabel('Groundwater Level',color="black",**csfont); plt.xlabel('Year',color="black",**csfont)
    ax.tick_params(axis='y', colors='black') ;ax.tick_params(axis='x', colors='black') 
    # plt.show()
    plt.savefig(path1+'\\%s_s.png'%station_info[station,0],dpi=300)
    plt.clf()
