...
cf = ax.contourf(X, Y, predictions_array, levels=np.linspace(min_value,max_value,40), cmap='bone_r', transform=ccrs.PlateCarree())
# cf = ax.contourf(X, Y, predictions_array, levels=np.linspace(min_value,max_value,40), cmap='Blues', transform=ccrs.PlateCarree())
...
output_path=r'D:\important\research\groundwater_forecast(monthly)\result\1st_layer\kriging figure\train\regional_predict\%s'%filename
...
ax.add_geometries(shp_file.geometries(), crs=ccrs.PlateCarree(), linewidths=1.2, edgecolor='k', facecolor='none') # edgecolor=調整邊框顏色
cf = ax.contourf(X, Y, obs_array, levels=np.linspace(min_value,max_value,40), cmap='bone_r', transform=ccrs.PlateCarree())
# cf = ax.contourf(X, Y, obs_array, levels=np.linspace(min_value,max_value,40), cmap='Blues', transform=ccrs.PlateCarree())
...
plt.title('obs time %s'%train_time[time], fontsize=20)
    # output_path=r'important\research\groundwater_forecast\python_code'
output_path=r'D:\important\research\groundwater_forecast(monthly)\result\1st_layer\kriging figure\train\regional_obs\%s'%filename
plt.savefig(os.path.join(output_path+'.png'), bbox_inches='tight')  # bbox_inches是修改成窄邊框
plt.close()
plt.cla()
plt.clf()
