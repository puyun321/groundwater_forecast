...
P_input = P_input[:-timestep]; P_input = np.array([np.sum(P_input[i-timestep:i,:],axis=0) for i in range(timestep-1,len(P_input))])
T_input = T_input[:-timestep]; T_input = np.array([np.sum(T_input[i-timestep:i],axis=0) for i in range(timestep-1,len(T_input))])
ETpot_input = ETpot_input[:-timestep]; ETpot_input = np.array([np.sum(ETpot_input[i-timestep:i],axis=0) for i in range(timestep-1,len(ETpot_input))])
...
G01_out1 = G01_new[timestep-1+forecast_timestep[0]:]  
G_train_out1 = G01_out1[:int(len(G01_out1)*0.8),:]
G01_out2 = G01_new[timestep-1+forecast_timestep[0]:]  
G_train_out2 = G01_out2[:int(len(G01_out2)*0.8),:]
G01_out3 = G01_new[timestep-1+forecast_timestep[0]:]  
...
optima.to_csv(r"D:\important\research\groundwater_forecast\python_code\puyun\result\optima_parameter(without_denorm).csv")
...
#%%
# save result
writer = pd.ExcelWriter(r"D:\important\research\groundwater_forecast\python_code\puyun\result\simulate(without_denorm).xlsx",engine='xlsxwriter')
pd.DataFrame(simulate_result).to_excel(writer,sheet_name="simulate_result")
