# -*- coding: utf-8 -*-
"""
Created on Fri Sep 16 16:59:22 2022
...
all_info=[];
for i in range(0,len(G2_station_info_new)):
...
G2_new_ = G2_new[timestep-1:-timestep]; G2_base = G2_new[timestep-2:-timestep-1]
forecast_timestep=np.array([timestep/3,timestep*2/3,timestep]).astype(int)

""" 
two types of groundwater variable, true value groundwater level and difference value groundwater level
For example, G2_out1_ observation groundwater level, G2_out1 obervation groundwater level difference between horizon t and t-1
"""

G2_out1_ = G2_new[timestep-2+forecast_timestep[0]:-(timestep-forecast_timestep[0])]; G2_out1=G2_out1_[1:,:]-G2_out1_[0:-1,:]
G2_out2_ = G2_new[timestep-2+forecast_timestep[1]:-(timestep-forecast_timestep[1])]; G2_out2=G2_out2_[1:,:]-G2_out2_[0:-1,:]
G2_out3_ = G2_new[timestep-2+forecast_timestep[2]:]; G2_out3 =G2_out3_[1:,:]-G2_out3_[0:-1,:]
...
G_train_obs = G2_new_[:int(len(G2_new_)*0.8),:]

G_train_out1 = G2_out1[:int(len(G2_out1)*0.8),:]
G_train_out2 = G2_out2[:int(len(G2_out2)*0.8),:]
G_train_out3 = G2_out3[:int(len(G2_out3)*0.8),:]
G_train_out = [G_train_out1, G_train_out2, G_train_out3]

G_train_out1_ = G2_out1_[1:len(G_train_out1)+1,:]
G_train_out2_ = G2_out2_[1:len(G_train_out2)+1,:]
G_train_out3_ = G2_out3_[1:len(G_train_out3)+1,:]
G_train_out_ = [G_train_out1_, G_train_out2_, G_train_out3_]
...
G_test_obs = G2_new_[int(len(G2_new_)*0.8):,:]

G_test_out1 = G2_out1[int(len(G2_out1)*0.8):,:]
G_test_out2 = G2_out2[int(len(G2_out2)*0.8):,:]
G_test_out3 = G2_out3[int(len(G2_out3)*0.8):,:]
G_test_out = [G_test_out1, G_test_out2, G_test_out3]

G_test_out1_ = G2_out1_[len(G_train_out1)+1:,:]
G_test_out2_ = G2_out2_[len(G_train_out2)+1:,:]
G_test_out3_ = G2_out3_[len(G_train_out3)+1:,:]
G_test_out_ = [G_test_out1_, G_test_out2_, G_test_out3_]
    
#%%
#shuffle train test    
shuffle_index = np.array(pd.read_excel(r"D:\important\research\groundwater_forecast\python_code\puyun\difference_level\result\1st_layer\hbvlstm(shuffle)-train(t+1).xlsx",sheet_name="shuffle_index").iloc[:,1])

P_shuffle_train_obs = P_input[shuffle_index[:int(len(shuffle_index)*0.8)],:]
T_shuffle_train_obs = T_input[shuffle_index[:int(len(shuffle_index)*0.8)],:]
ETpot_shuffle_train_obs = ETpot_input[shuffle_index[:int(len(shuffle_index)*0.8)],:]
G_shuffle_train_obs = G2_new_[shuffle_index[:int(len(shuffle_index)*0.8)],:]

G_shuffle_train_out1 = G2_out1[shuffle_index[:int(len(shuffle_index)*0.8)],:]
G_shuffle_train_out2 = G2_out2[shuffle_index[:int(len(shuffle_index)*0.8)],:]
G_shuffle_train_out3 = G2_out3[shuffle_index[:int(len(shuffle_index)*0.8)],:]
G_shuffle_train_out = [G_train_out1, G_train_out2, G_train_out3]

G_shuffle_train_out1_ = G_train_out1_[shuffle_index[:int(len(shuffle_index)*0.8)],:]
G_shuffle_train_out2_ = G_train_out2_[shuffle_index[:int(len(shuffle_index)*0.8)],:]
G_shuffle_train_out3_ = G_train_out3_[shuffle_index[:int(len(shuffle_index)*0.8)],:]
G_shuffle_train_out_ = [G_shuffle_train_out1_, G_shuffle_train_out2_, G_shuffle_train_out3_]

P_shuffle_test_obs = P_input[shuffle_index[int(len(shuffle_index)*0.8):],:]
T_shuffle_test_obs = T_input[shuffle_index[int(len(shuffle_index)*0.8):],:]
ETpot_shuffle_test_obs = ETpot_input[shuffle_index[int(len(shuffle_index)*0.8):],:]
G_shuffle_test_obs = G2_new_[shuffle_index[int(len(shuffle_index)*0.8):],:]

G_shuffle_test_out1 = G2_out1[shuffle_index[int(len(shuffle_index)*0.8):],:]
G_shuffle_test_out2 = G2_out2[shuffle_index[int(len(shuffle_index)*0.8):],:]
G_shuffle_test_out3 = G2_out3[shuffle_index[int(len(shuffle_index)*0.8):],:]
G_shuffle_test_out = [G_train_out1, G_train_out2, G_train_out3]

G_shuffle_test_out1_ = G_train_out1_[shuffle_index[int(len(shuffle_index)*0.8):],:]
G_shuffle_test_out2_ = G_train_out2_[shuffle_index[int(len(shuffle_index)*0.8):],:]
G_shuffle_test_out3_ = G_train_out3_[shuffle_index[int(len(shuffle_index)*0.8):],:]
G_shuffle_test_out_ = [G_shuffle_test_out1_, G_shuffle_test_out2_, G_shuffle_test_out3_]

#%%
from scipy.optimize import differential_evolution

# parameter range https://agupubs.onlinelibrary.wiley.com/doi/10.1029/2019JD031485
bounds = [(1,6), (50, 1000), (0.2,1), (0,0.2), (0.05, 0.9), (0, 100), (0,10)]
optima_alltimestep=[]
for j in range(0,3):
    optima=[[]*1 for i in range(0,len(G_train_obs[0]))]
    for station in range(0,len(G_train_obs[0])):
        # G_in=G_train_obs[:,station]    
        P_in,T_in,ETpot_in=P_shuffle_train_obs[:,station],T_shuffle_train_obs[:,station],ETpot_shuffle_train_obs[:,station]
        G_out=G_shuffle_train_out_[j][:,station]
...
for index,optima in enumerate(optima_alltimestep):
    ...
    optima.columns = ['parBETA','parFC','parLP','parPCORR','parK0','parUZL','parPERC0']
    optima.to_csv(r"D:\important\research\groundwater_forecast\python_code\puyun\difference_level\result\2nd_layer\optima_parameter(t+%s)-difference.csv"%(index+1))
...
for index in range(0,3):
    optima = pd.read_csv(r"D:\important\research\groundwater_forecast\python_code\puyun\difference_level\result\2nd_layer\optima_parameter(t+%s)-difference.csv"%(index+1),index_col=0)
    location=1
    
    def HBV_trained_model(G_input,P_input,T_input,ETpot_input):
        parTT,parCFMAX,parSFCF,parCFR,parCWH = 0,0,0,0,0
        ...
        for station in range(0,len(optima)):            
            ...
            for t in range(0, train_len):      
                ...
                parK0 = optima.iloc[station,4]
                parUZL = optima.iloc[station,5]
                parPERC0 = optima.iloc[station,6]
                # Separate precipitation into liquid and solid components
                PRECIP = P_in[t] * parPCORR
                RAIN = np.multiply(PRECIP, T_in[t] >= parTT)
                SNOW = np.multiply(PRECIP, T_in[t] < parTT)
                ...
                GW_forecast = SUZ+SUZ_[t]
    
                simulate_result[station].append(GW_forecast)  
    
                # simulate_result[station].append(SM)        
        simulate_result=np.transpose(np.array(simulate_result))
        return simulate_result
    simulate_train_result=HBV_trained_model(G_shuffle_train_obs,P_shuffle_train_obs,T_shuffle_train_obs,ETpot_shuffle_train_obs)
    simulate_test_result=HBV_trained_model(G_shuffle_test_obs,P_shuffle_test_obs,T_shuffle_test_obs,ETpot_shuffle_test_obs)
...
g1_station_num=G2_new.shape[1]
sorted_train_new=[]
for i in range(0,3):
    train = all_simulate_train_result[i]
    test = all_simulate_test_result[i]
    train_test=np.concatenate([train,test])
    sorted_train=[[]*1 for i in range(0,len(train)+len(test))]
    for j in range(0,len(shuffle_index)):
        sorted_train[shuffle_index[j]].append(train_test[j,:])
...
writer = pd.ExcelWriter(r"D:\important\research\groundwater_forecast\python_code\puyun\difference_level\result\2nd_layer\simulate-shuffle(train).xlsx",engine='xlsxwriter')
for i,simulate_train_result in enumerate(all_simulate_train_result):
    ...
    pd.DataFrame(G_shuffle_train_out_[i]).to_excel(writer,sheet_name="groundwater_observation(t+%s)"%(i+1))

train_R2=[[]*1 for i in range(0,3)];train_rmse=[[]*1 for i in range(0,3)]
for i in range(0,3):
    for j in range(0,G_shuffle_train_out[i].shape[1]):
        pred=pd.DataFrame(all_simulate_train_result[i][:,j])
        obs=pd.DataFrame(G_shuffle_train_out_[i][:,j])
...
writer = pd.ExcelWriter(r"D:\important\research\groundwater_forecast\python_code\puyun\difference_level\result\2nd_layer\simulate-shuffle(test).xlsx",engine='xlsxwriter')
for i,simulate_train_result in enumerate(all_simulate_train_result):
    pd.DataFrame(simulate_test_result).to_excel(writer,sheet_name="simulate_result(t+%s)"%(i+1))
    pd.DataFrame(G_shuffle_test_out_[i]).to_excel(writer,sheet_name="groundwater_observation(t+%s)"%(i+1))

test_R2=[[]*1 for i in range(0,3)];test_rmse=[[]*1 for i in range(0,3)]
for i in range(0,3):
    for j in range(0,G_shuffle_test_out[i].shape[1]):
        pred=pd.DataFrame(all_simulate_test_result[i][:,j])
        obs=pd.DataFrame(G_shuffle_test_out_[i][:,j])
