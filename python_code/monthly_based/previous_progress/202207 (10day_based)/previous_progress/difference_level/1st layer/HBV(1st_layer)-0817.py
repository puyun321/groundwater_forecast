...
for index in range(0,3):
    ...
    simulate_train_result=HBV_trained_model(G_train_obs,P_train_obs,T_train_obs,ETpot_train_obs)
...
writer = pd.ExcelWriter(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\simulate(train).xlsx",engine='xlsxwriter')
for i,simulate_train_result in enumerate(all_simulate_train_result):
    ...
    pd.DataFrame(G_train_out_[i]).to_excel(writer,sheet_name="groundwater_observation(t+%s)"%(i+1))
...
for i in range(0,3):
    for j in range(0,G_train_out[i].shape[1]):
        ...
        obs=pd.DataFrame(G_train_out_[i][:,j])
...
for i,simulate_train_result in enumerate(all_simulate_train_result):
    ...
    pd.DataFrame(G_test_out_[i]).to_excel(writer,sheet_name="groundwater_observation(t+%s)"%(i+1))
...
for i in range(0,3):
    for j in range(0,G_test_out[i].shape[1]):
        ...
        obs=pd.DataFrame(G_test_out_[i][:,j])
