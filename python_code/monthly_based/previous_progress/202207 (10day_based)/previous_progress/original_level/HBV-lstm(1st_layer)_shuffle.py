...
simulate_shuffle_train = simulate_merge_train[shuffle_index[:int(len(shuffle_index)*0.8)],:]
simulate_shuffle_train = np.concatenate([simulate_shuffle_train[:,:,i] for i in range(0,len(simulate_shuffle_train[0,0,:]))])

simulate_shuffle_test = simulate_merge_train[shuffle_index[int(len(shuffle_index)*0.8):],:,:]
...
save_path=r"D:\important\research\groundwater_forecast\python_code\puyun\original_level\model\1st_layer\hbv-lstm(shuffle).hdf5"
...
for i in range(0,3):
    for station in range(0,g1_station_num):
        ...
        train_new_R2[i].append(error_indicator.np_R2(np.squeeze(G_train_output[shuffle_index[:int(len(shuffle_index)*0.8)],forecast_timestep[i],station]),pred_train_new[i][:,station]))
        train_new_rmse[i].append(error_indicator.np_RMSE(np.squeeze(G_train_output[shuffle_index[:int(len(shuffle_index)*0.8)],forecast_timestep[i],station]),pred_train_new[i][:,station]))
        test_new_R2[i].append(error_indicator.np_R2(np.squeeze(G_train_output[shuffle_index[int(len(shuffle_index)*0.8):],forecast_timestep[i],station]),pred_test_new[i][:,station]))
        test_new_rmse[i].append(error_indicator.np_RMSE(np.squeeze(G_train_output[shuffle_index[int(len(shuffle_index)*0.8):],forecast_timestep[i],station]),pred_test_new[i][:,station]))


for i in range(0,3):
    writer = pd.ExcelWriter(r"D:\important\research\groundwater_forecast\python_code\puyun\original_level\result\1st_layer\hbvlstm(shuffle)-forecast(t+%s).xlsx"%(i+1),engine='xlsxwriter')
