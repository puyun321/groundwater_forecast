...
hbv_lstm=pd.read_excel(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\sorted_predict(hbv-lstm).xlsx",sheet_name="t+1(shuffle)",index_col=0)
hbv=pd.read_excel(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\sorted_predict.xlsx",sheet_name="t+1(shuffle)",index_col=0).iloc[1:,:]
...
path='D:\\important\\research\\groundwater_forecast\\python_code\\puyun\\result\\1st_layer\\time series figure\\%s'%start_date
...
# station=28
    ...
    plt.plot(choose_date,model_train_output[select_index,station], color='black')
    plt.plot(choose_date,hbv_lstm.iloc[select_index,station], color='red')
    plt.plot(choose_date,hbv.iloc[select_index,station], color='blue')
    plt.legend(labels=['Observed data','HBV-LSTM','HBV'],loc='upper right')
    plt.ylabel('Groundwater Level'); plt.xlabel('Groundwater Level')
    # plt.show()
    plt.savefig(path+'\\%s.png'%station_info[station,0])