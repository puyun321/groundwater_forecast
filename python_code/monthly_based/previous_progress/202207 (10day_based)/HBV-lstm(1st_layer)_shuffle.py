...
save_path=r"D:\important\research\groundwater_forecast\python_code\puyun\model\1st_layer\hbv-lstm(shuffle).hdf5"
...
for i in range(0,3):
    writer = pd.ExcelWriter(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\hbvlstm(shuffle)-forecast(t+%s).xlsx"%(i+1),engine='xlsxwriter')
...
for i in range(0,3):
    writer = pd.ExcelWriter(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\hbvlstm(shuffle)-test(t+%s).xlsx"%(i+1),engine='xlsxwriter')
    pd.DataFrame(performance.iloc[i,:]).to_excel(writer,sheet_name="all_performance")
    pd.DataFrame(pred_test_new[i]).to_excel(writer,sheet_name="test_eachstation")
    pd.DataFrame(test_new_rmse[i]).to_excel(writer,sheet_name="test_rmse_eachstation")                
    pd.DataFrame(test_new_R2[i]).to_excel(writer,sheet_name="test_R2_eachstation")
...
split_index=int(0.8*len(G_train_output))
g1_station_num=G01_new.shape[1]
writer = pd.ExcelWriter(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\sorted_predict(hbv-lstm).xlsx",engine='xlsxwriter')
sorted_train_new=[]
for i in range(0,3):
    train=convert_result(pred_train[:,i],g1_station_num)
    test=convert_result(pred_test[:,i],g1_station_num)
