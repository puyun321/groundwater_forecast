...
G2 = pd.read_csv(r"D:\important\research\groundwater_forecast\daily_data\groundwater_l2.csv",delimiter=',');G2['date'] = pd.to_datetime(G2['date']);
...
G3 = pd.read_csv(r"D:\important\research\groundwater_forecast\daily_data\groundwater_l3.csv",delimiter=',');G3['date'] = pd.to_datetime(G3['date']);
...
G4 = pd.read_csv(r"D:\important\research\groundwater_forecast\daily_data\groundwater_l4.csv",delimiter=',');G4['date'] = pd.to_datetime(G4['date']);
...
G01_new_ = G01_new[timestep-1:-timestep]   
forecast_timestep=np.array([timestep/3,timestep*2/3,timestep]).astype(int)
G01_out1 = G01_new[timestep-1+forecast_timestep[0]:-(timestep-forecast_timestep[0])]; out_difference1=G01_out1[1:,:]-G01_out1[0:-1,:]
...
G_train_obs = G01_new_[train_index,:]

G_train_out1 = G01_out1[train_index,:]
G_train_out2 = G01_out2[train_index,:]
G_train_out3 = G01_out3[train_index,:]
...
G_test_obs = G01_new_[test_index,:]

G_test_out1 = G01_out1[test_index,:]
G_test_out2 = G01_out2[test_index,:]
...
for index,optima in enumerate(optima_alltimestep):
    ...
    optima.to_csv(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\optima_parameter(t+%s).csv"%(index+1))
...
for index in range(0,3):
    optima = pd.read_csv(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\optima_parameter(t+%s).csv"%(index+1),index_col=0)
    location=1
...
writer = pd.ExcelWriter(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\sorted_predict.xlsx",engine='xlsxwriter')
sorted_train_new=[]
for i in range(0,3):
    ...
    real_test = all_simulate_real_test_result[i]
...
writer = pd.ExcelWriter(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\simulate-shuffle(train).xlsx",engine='xlsxwriter')
...
writer = pd.ExcelWriter(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\simulate-shuffle(test).xlsx",engine='xlsxwriter')
...
writer = pd.ExcelWriter(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\simulate(test).xlsx",engine='xlsxwriter')
...
for i in range(0,3):
    for j in range(0,G_test_out[i].shape[1]):
        pred=pd.DataFrame(all_simulate_real_test_result[i][:,j])
