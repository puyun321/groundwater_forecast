...
obs_test=pd.read_excel(r"D:\important\research\groundwater_forecast(monthly)\result\1st_layer\sorted_observation.xlsx",sheet_name="t+1(test)",index_col=0)
pred_test=pd.read_excel(r"D:\important\research\groundwater_forecast(monthly)\result\1st_layer\sorted_predict(hbv-ann).xlsx",sheet_name="t+1(test)",index_col=0)
simulate_test=(pd.read_excel(r"D:\important\research\groundwater_forecast(monthly)\result\1st_layer\sorted_predict.xlsx",sheet_name="t+1(test)",index_col=0)).iloc[1:,:]
...
time = G0_.index; test_time = time[-len(obs_test):]
...
for time in range(0,len(pred_test)):
    ...
    output_path=r'D:\important\research\groundwater_forecast(monthly)\result\1st_layer\kriging figure\test\regional_predict\%s'%filename
    ...
    plt.title('obs time %s'%filename, fontsize=20)
        # output_path=r'important\research\groundwater_forecast\python_code'
    output_path=r'D:\important\research\groundwater_forecast(monthly)\result\1st_layer\kriging figure\test\regional_obs\%s'%filename
    plt.savefig(os.path.join(output_path+'.png'), bbox_inches='tight')  # bbox_inches是修改成窄邊框
    plt.close()
    gc.collect()  # 清理站存記