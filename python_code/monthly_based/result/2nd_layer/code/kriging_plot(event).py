...
obs_train=pd.read_excel(r"D:\important\research\groundwater_forecast(monthly)\result\2nd_layer\sorted_observation.xlsx",sheet_name="%s(train)"%forecast_timestep,index_col=0)
pred_train=pd.read_excel(r"D:\important\research\groundwater_forecast(monthly)\result\2nd_layer\sorted_predict(hbv-ann).xlsx",sheet_name="%s(shuffle)"%forecast_timestep,index_col=0)
simulate_train=(pd.read_excel(r"D:\important\research\groundwater_forecast(monthly)\result\2nd_layer\sorted_predict.xlsx",sheet_name="%s(shuffle)"%forecast_timestep,index_col=0)).iloc[1:,:]
...
output_path=r'D:\important\research\groundwater_forecast(monthly)\result\2nd_layer\kriging event figure\%s\train\regional_predict\%s'%(forecast_timestep,filename)
...
plt.title('simulate time %s (RMSE=%s , R2=%s)'%(filename,simulate_train_RMSE,simulate_train_R2), fontsize=20)
    # output_path=r'important\research\groundwater_forecast\python_code'
output_path=r'D:\important\research\groundwater_forecast(monthly)\result\2nd_layer\kriging event figure\%s\train\regional_simulate\%s'%(forecast_timestep,filename)
plt.savefig(os.path.join(output_path+'.png'), bbox_inches='tight')  # bbox_inches是修改成窄邊框
plt.close()
plt.cla()
plt.clf()
...
plt.title('obs time %s (%s,%s)'%(filename,obs_mean,obs_std), fontsize=20)
    # output_path=r'important\research\groundwater_forecast\python_code'
output_path=r'D:\important\research\groundwater_forecast(monthly)\result\2nd_layer\kriging event figure\%s\train\regional_obs\%s'%(forecast_timestep,filename)
plt.savefig(os.path.join(output_path+'.png'), bbox_inches='tight')  # bbox_inches是修改成窄邊框
plt.close()
plt.cla()
plt.clf()
