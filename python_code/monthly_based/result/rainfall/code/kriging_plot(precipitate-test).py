...
test_initial_date='2017-01-01'
test_date=(date[(date['date']>=test_initial_date)]).reset_index(drop=True)
...
choose_date = (test_date[(test_date['date']>=start_date) & (test_date['date']<=end_date)].iloc[:,0]).reset_index(drop=True)
select_index= test_date[(test_date['date']>=start_date) & (test_date['date']<=end_date)].index
...
output_path=r'result\rainfall\test'
