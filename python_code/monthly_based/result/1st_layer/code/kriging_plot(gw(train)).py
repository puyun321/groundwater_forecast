...
pred_train_z = [Ordinary_Kriging(np.squeeze(pred_train.iloc[i, 1:]),G_station_info,G_grid_lon,G_grid_lat)[0] for i in range(0,len(pred_train))]
pred_train_z = np.nan_to_num(pred_train_z)

obs_train_z = [Ordinary_Kriging(np.squeeze(obs_train.iloc[i, 1:]),G_station_info,G_grid_lon,G_grid_lat)[0] for i in range(0,len(obs_train))]
...
for time in range(0,len(pred_train)):
    ...
    output_path=r'result\1st_layer\kriging event figure\train\pred'
    ...
    plt.title('obs time %s'%filename, fontsize=20)
        # output_path=r'important\research\groundwater_forecast\python_code'
    output_path=r'result\1st_layer\kriging event figure\train\obs'
    isExist = os.path.exists(output_path)
    if not isExist:
      # Create a new directory because it does not exist 
      os.makedirs(output_path)
