...
for time in range(0,len(performance_z)):
    mean_z = performance_z[time,:,:]
    one_z = performance_1_z[time,:,:]
    two_z = performance_2_z[time,:,:]
    three_z = performance_3_z[time,:,:]
    ...
    cf = ax.contourf(X, Y, mean_z, levels=np.linspace(min_value,max_value,40), cmap='autumn_r', transform=ccrs.PlateCarree())
    ...
    output_path=r'result\1st_layer\kriging season figure\mean'
    ...
    shp_file = shpreader.Reader(r'zhuoshui(shp)\zhuoshui.shp')
    ax.add_geometries(shp_file.geometries(), crs=ccrs.PlateCarree(), linewidths=1.2, edgecolor='k', facecolor='none') # edgecolor=調整邊框顏色
    cf = ax.contourf(X, Y, one_z, levels=np.linspace(min_value,max_value,40), cmap='autumn_r', transform=ccrs.PlateCarree())
    shp_clip(cf, ax, r'zhuoshui(shp)\zhuoshui.shp')
    plt.rcParams['font.sans-serif'] =['Taipei Sans TC Beta']  #匯入中文字體
    cb = plt.colorbar(cf)
    plt.rcParams['font.sans-serif'] = ['Microsoft JhengHei'] 
    plt.rcParams['axes.unicode_minus'] = False
    plt.title('%s'%filename, fontsize=20)
    output_path=r'result\1st_layer\kriging season figure\one'
    isExist = os.path.exists(output_path)
    if not isExist:
      # Create a new directory because it does not exist 
      os.makedirs(output_path)
    ...
    shp_file = shpreader.Reader(r'zhuoshui(shp)\zhuoshui.shp')
    ax.add_geometries(shp_file.geometries(), crs=ccrs.PlateCarree(), linewidths=1.2, edgecolor='k', facecolor='none') # edgecolor=調整邊框顏色
    cf = ax.contourf(X, Y, two_z, levels=np.linspace(min_value,max_value,40), cmap='autumn_r', transform=ccrs.PlateCarree())
    shp_clip(cf, ax, r'zhuoshui(shp)\zhuoshui.shp')
    plt.rcParams['font.sans-serif'] =['Taipei Sans TC Beta']  #匯入中文字體
    cb = plt.colorbar(cf)
    plt.rcParams['font.sans-serif'] = ['Microsoft JhengHei'] 
    plt.rcParams['axes.unicode_minus'] = False
    plt.title('%s'%filename, fontsize=20)
    output_path=r'result\1st_layer\kriging season figure\two'
    isExist = os.path.exists(output_path)
    if not isExist:
      # Create a new directory because it does not exist 
      os.makedirs(output_path)
    ...
    shp_file = shpreader.Reader(r'zhuoshui(shp)\zhuoshui.shp')
    ax.add_geometries(shp_file.geometries(), crs=ccrs.PlateCarree(), linewidths=1.2, edgecolor='k', facecolor='none') # edgecolor=調整邊框顏色
    cf = ax.contourf(X, Y, three_z, levels=np.linspace(min_value,max_value,40), cmap='autumn_r', transform=ccrs.PlateCarree())
    shp_clip(cf, ax, r'zhuoshui(shp)\zhuoshui.shp')
    plt.rcParams['font.sans-serif'] =['Taipei Sans TC Beta']  #匯入中文字體
    cb = plt.colorbar(cf)
    plt.rcParams['font.sans-serif'] = ['Microsoft JhengHei'] 
    plt.rcParams['axes.unicode_minus'] = False
    plt.title('%s'%filename, fontsize=20)
    output_path=r'result\1st_layer\kriging season figure\three'
    isExist = os.path.exists(output_path)
    if not isExist:
      # Create a new directory because it does not exist 
      os.makedirs(output_path)
...
train_hbvlstm3=pd.read_excel(r"result\1st_layer\performance_comparison\train\T+3.xlsx",sheet_name="HBV-AE-LSTM")
