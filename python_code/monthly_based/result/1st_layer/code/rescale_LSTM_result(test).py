...
#%%
forecast_timestep=3
pred_test=(pd.read_excel(r"result\1st_layer\predict(hbv-ae-ann)2.xlsx",sheet_name="pred_test_%s"%(forecast_timestep)).iloc[1:,1:]).reset_index(drop=True)
simulate_test=(pd.read_excel(r"result\1st_layer\sorted_predict.xlsx",sheet_name="t+%s(test)"%(forecast_timestep)).iloc[:-1,1:]).reset_index(drop=True)
obs_test=(pd.read_excel(r"result\1st_layer\predict(hbv-ae-ann)2.xlsx",sheet_name="obs_test_%s"%(forecast_timestep)).iloc[1:,1:]).reset_index(drop=True)

#%%
"""Adjust forecast"""
adjust_ratio = pred_test.iloc[:-1,:].reset_index(drop=True)/obs_test.iloc[:-1,:].reset_index(drop=True)
pred_test_adjust = (pred_test.iloc[1:,:].reset_index(drop=True))/adjust_ratio
pred_mean = np.mean(pred_test_adjust,axis=0)
pred_std = np.std(pred_test_adjust,axis=0)

for j in range(0,pred_test_adjust.shape[1]):
    for i in range(0,len(pred_test_adjust)):
        if pred_test_adjust.iloc[i,j]>pred_mean[j]+3*pred_std[j] or pred_test_adjust.iloc[i,j]<pred_mean[j]-3*pred_std[j]:
            pred_test_adjust.iloc[i,j]=np.nan
...
pred_test_adjust=pd.DataFrame(interpolate(pred_test_adjust))
simulate_test=simulate_test.iloc[1:,:].reset_index(drop=True)
obs_test=obs_test.iloc[1:,:].reset_index(drop=True)
...
test_date=(date[(date['date']>=start_date)]).reset_index(drop=True)
choose_date = (test_date.iloc[forecast_timestep:-4+forecast_timestep]).reset_index(drop=True)

#%%
"""performance compare"""

pred_R2=[];simulate_R2=[];pred_RMSE=[];simulate_RMSE=[]
for i in range(0,pred_test_adjust.shape[1]):
    pred_R2.append(error_indicator.np_R2(obs_test.iloc[:,i],pred_test_adjust.iloc[:,i]))
    simulate_R2.append(error_indicator.np_R2(obs_test.iloc[:,i],simulate_test.iloc[:,i]))
    pred_RMSE.append(error_indicator.np_RMSE(obs_test.iloc[:,i],pred_test_adjust.iloc[:,i]))
    simulate_RMSE.append(error_indicator.np_RMSE(obs_test.iloc[:,i],simulate_test.iloc[:,i])) 
...
path2='result\\1st_layer\\time series figure\\test\\T+%s\\%s'%(forecast_timestep,start_date)
...
def plot_figure(performance,name):
    ...
    path3 ='result\\1st_layer\\performance figure\\test\\T+%s'%(forecast_timestep)
...
# plot_figure(all_performance.iloc[:,3],'Simulate Testing rmse(T+1)')
