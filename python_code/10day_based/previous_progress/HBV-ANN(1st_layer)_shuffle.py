...
def DNN_model2(timestep):
    ...
    final_output = Conv1D(6,kernel_size=3)(final_output)
    final_output = Conv1D(6,kernel_size=3)(final_output)    
    # final_output = LSTM(6,stateful=False,return_sequences=True)(final_output)
    # final_output = LSTM(6,stateful=False,return_sequences=False)(final_output)
...
#%%    
fixed_parameter_1=(pd.read_csv(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\optima_parameter(t+1).csv",index_col=0)).astype(np.float32)
fixed_parameter_2=(pd.read_csv(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\optima_parameter(t+2).csv",index_col=0)).astype(np.float32)
fixed_parameter_3=(pd.read_csv(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\optima_parameter(t+3).csv",index_col=0)).astype(np.float32)
...
G_train_input = G_multi_input_origin[train_index[1:] ,:]
...
simulate_train=[]
simulate_train.append(np.array(pd.read_excel(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\sorted_predict.xlsx",sheet_name="t+1(shuffle)",index_col=0)))
simulate_train.append(np.array(pd.read_excel(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\sorted_predict.xlsx",sheet_name="t+2(shuffle)",index_col=0)))
simulate_train.append(np.array(pd.read_excel(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\sorted_predict.xlsx",sheet_name="t+3(shuffle)",index_col=0)))

simulate_test=[]
simulate_test.append(np.array(pd.read_excel(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\sorted_predict.xlsx",sheet_name="t+1(test)",index_col=0)))
simulate_test.append(np.array(pd.read_excel(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\sorted_predict.xlsx",sheet_name="t+2(test)",index_col=0)))
simulate_test.append(np.array(pd.read_excel(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\sorted_predict.xlsx",sheet_name="t+3(test)",index_col=0)))
...
save_path=r"D:\important\research\groundwater_forecast\python_code\puyun\model\1st_layer\hbv-ann(shuffle).hdf5"
...
for i in range(0,3):
    writer = pd.ExcelWriter(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\hbvann(shuffle)-forecast(t+%s).xlsx"%(i+1),engine='xlsxwriter')
...
G_test_input = G_multi_input[test_index,:]
...
for i in range(0,3):
    writer = pd.ExcelWriter(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\hbvann(shuffle)-test(t+%s).xlsx"%(i+1),engine='xlsxwriter')
    pd.DataFrame(performance.iloc[i,:]).to_excel(writer,sheet_name="all_performance")
    pd.DataFrame(pred_test_new[i]).to_excel(writer,sheet_name="test_eachstation")
    pd.DataFrame(test_new_rmse[i]).to_excel(writer,sheet_name="test_rmse_eachstation")                
    pd.DataFrame(test_new_R2[i]).to_excel(writer,sheet_name="test_R2_eachstation")
...
split_index=int(0.8*len(G_train_output))
g1_station_num=G01_new.shape[1]
writer = pd.ExcelWriter(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\sorted_predict(hbv-ann).xlsx",engine='xlsxwriter')
sorted_train_new=[]
for i in range(0,3):
    train=convert_result(pred_train[:,i],g1_station_num)
    test=convert_result(pred_test[:,i],g1_station_num)
