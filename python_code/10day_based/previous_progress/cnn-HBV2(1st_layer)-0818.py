...
class HBV_layer(Layer):
    ...
    def call(self,tensor):
        ...
        (P_obs, T_obs, ETpot_obs, G_obs_in, fixed_parameter1,fixed_parameter2,fixed_parameter3) = tf.split(tensor, num_or_size_splits=[1,1,1,1,4,4,4], axis=1)
...
def DNN_model_2(timestep):
    
    inputs1 = Input(shape=(timestep,))    
    # output1 = Lambda(lambda x: tf.expand_dims(x, -1))(inputs1)
    # output1 = Conv1D(9,kernel_size=3,padding='same')(output1)
    # output1 = Conv1D(9,kernel_size=3,padding='same')(output1)    
    # output1 = Flatten()(output1)
    output1 = Dense(3, activation='relu')(inputs1)
    
    inputs2 = Input(shape=(16,))
    simulate_layer = HBV_layer()(inputs2)
    simulate_layer = Dense(3, activation='relu')(simulate_layer)
    ...
    final_output = Dense(6, activation='relu')(final_output)
...
if __name__ == '__main__':
    ...
    fixed_parameter_1=(pd.read_csv(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\optima_parameter(t+1)-difference.csv",index_col=0)).astype(np.float32)
    fixed_parameter_2=(pd.read_csv(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\optima_parameter(t+2)-difference.csv",index_col=0)).astype(np.float32)
    fixed_parameter_3=(pd.read_csv(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\optima_parameter(t+3)-difference.csv",index_col=0)).astype(np.float32)
    ...
    model=DNN_model_2(timestep)
    learning_rate=1e-3
    adam = Adam(learning_rate=learning_rate)
    model.compile(optimizer=adam,loss='mse')
    earlystopper = EarlyStopping(monitor='val_loss', patience=100, verbose=0)        
    save_path=r"D:\important\research\groundwater_forecast\python_code\puyun\model\1st_layer\dcnn.hdf5"
    ...
    for i in range(0,3):
        writer = pd.ExcelWriter(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\cnnhbv-forecast(t+%s).xlsx"%(i+1),engine='xlsxwriter')
