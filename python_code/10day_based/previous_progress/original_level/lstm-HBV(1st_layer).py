...
G01_new_=[]; SM_min=[[]*1 for i in range(0,G01_new.shape[1])]
for i in range(0,G01_new.shape[1]):
    if np.any(G01_new[:,i]<0)==True:
        SM_min[i]=np.min(G01_new[:,i])
        G01_new_.append(G01_new[:,i]-SM_min[i])
    else:
        G01_new_.append(G01_new[:,i])
G01_new_=np.array(G01_new_).T
...
class HBV_layer(Layer):
    ...
    def call(self,tensor):
        ...
        def HBV_model(parPCORR,parLP,parBETA,parFC):
            ...
            SM = tf.cast(G_obs_in, dtype=tf.float32)
...
def DNN_model_1(timestep):
    
    inputs1 = Input(shape=(timestep,))
    output1 = Lambda(lambda x: tf.expand_dims(x, -1))(inputs1)
    # output1=LSTM(9,stateful=False,return_sequences=True)(output1)
    ...
    inputs2 = Input(shape=(16,))
    # inputs2 = Lambda(lambda x: tf.expand_dims(x, -1))(inputs2)

    concat_input = Concatenate(axis=-1)([output1,inputs2]);

    simulate_layer = HBV_layer()(concat_input)
    
    final_output = Average()([output1,simulate_layer])
    # final_output = Concatenate(axis=-1)([output1,simulate_layer])
    final_output = Dense(8, activation='linear')(final_output)
...
if __name__ == '__main__':
    ...
    preprocessing_module=multi_input_output(G01_new_,input_timestep=timestep,output_timestep=timestep)
    ...
    model_train_input_2=np.concatenate([np.expand_dims(P_train_obs,axis=1),np.expand_dims(T_train_obs,axis=1),np.expand_dims(ETpot_train_obs,axis=1),
    ...
    """ model construction """ 
    model=DNN_model_1(timestep)
    learning_rate=1e-4
    adam = Adam(learning_rate=learning_rate)
    model.compile(optimizer=adam,loss='mse')
    earlystopper = EarlyStopping(monitor='val_loss', patience=100, verbose=0)        
    save_path=r"D:\important\research\groundwater_forecast\python_code\puyun\original_level\model\1st_layer\dlstm.hdf5"
    checkpoint =ModelCheckpoint(save_path,save_best_only=True)
    callback_list=[earlystopper,checkpoint]        
    model.fit([model_train_input_1,model_train_input_2], model_train_output, epochs=100, batch_size=32,validation_split=0.2,callbacks=callback_list,shuffle=True)
    ...
    model_test_input_2=np.concatenate([np.expand_dims(P_test_obs,axis=1),np.expand_dims(T_test_obs,axis=1),np.expand_dims(ETpot_test_obs,axis=1),
...
#%%    
    pred_train_new_=[[]*1 for i in range(0,3)]; pred_test_new_=[[]*1 for i in range(0,3)]
    for i in range(0,len(SM_min)):
        if np.size(SM_min[i])>0:
            for j in range(0,3):
                pred_train_new_[j].append(pred_train_new[j][:,i]+SM_min[i])
                pred_test_new_[j].append(pred_test_new[j][:,i]+SM_min[i])
                
        else:
            for j in range(0,3):
                pred_train_new_[j].append(pred_train_new[j][:,i])            
                pred_test_new_[j].append(pred_test_new[j][:,i])

            
    for i in range(0,3):
        pred_train_new_[i]=np.array(pred_train_new_[i]).T
        pred_test_new_[i]=np.array(pred_test_new_[i]).T
...
#%%   
    ...
    for i in range(0,3):
        writer = pd.ExcelWriter(r"D:\important\research\groundwater_forecast\python_code\puyun\original_level\result\1st_layer\lstmhbv-forecast(t+%s).xlsx"%(i+1),engine='xlsxwriter')
        pd.DataFrame(performance.iloc[i,:]).to_excel(writer,sheet_name="all_performance")
        pd.DataFrame(pred_train_new_[i]).to_excel(writer,sheet_name="train_eachstation")
        pd.DataFrame(pred_test_new_[i]).to_excel(writer,sheet_name="test_eachstation")
