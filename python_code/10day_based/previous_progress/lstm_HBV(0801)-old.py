...
class HBV_1st_layer(Layer):
    def __init__(self, units=32, input_dim=1):
        super(HBV_1st_layer, self).__init__()
        # parPCORR,parBETA,parFC  = tf.random_normal_initializer()
        self.parPCORR = self.add_weight(
            shape=(input_dim, units), initializer="random_normal", trainable=True)
        self.parLP = self.add_weight(
            shape=(input_dim, units), initializer="random_normal", trainable=True)
        self.parBETA = self.add_weight(
            shape=(input_dim, units), initializer="random_normal", trainable=True)
        self.parFC = self.add_weight(
            shape=(input_dim, units), initializer="random_normal", trainable=True)
    ...
    def call(self,tensor):
        ...
        SNOWPACK = tf.add(tf.zeros(tf.shape(self.parBETA), dtype=tf.float32),0.001)
        MELTWATER = tf.add(tf.zeros(tf.shape(self.parBETA), dtype=tf.float32),0.001)
        SM = G_obs_in
        ETact = tf.add(tf.zeros(tf.shape(self.parBETA), dtype=tf.float32),0.001)
        # Apply correction factor to precipitation
        P =  tf.multiply(self.parPCORR, P_obs)
        
        """ Separate precipitation into liquid and solid components """
    
        PRECIP = tf.multiply(P, self.parPCORR)
        ...
        soil_wetness = tf.pow(tf.divide(SM, self.parFC), self.parBETA)
        ...
        excess = tf.subtract(SM,self.parFC)
        excess = tf.clip_by_value(excess,0.0,excess)
        SM = tf.subtract(SM,excess)
        evapfactor = tf.divide(SM, tf.multiply(self.parLP, self.parFC))
...
def DNN_model_1(timestep,P_obs,T_obs,ETpot_obs,G_obs):
    ...
    simulate_layer = HBV_1st_layer()(output)
    final_output = Concatenate(axis=-1)([output_1,simulate_layer]);
    final_output = Dense(1, activation='linear')(final_output)

    model = Model(inputs=[inputs1,inputs2,inputs3,inputs4,inputs5], outputs=simulate_layer)
...
if __name__ == '__main__':
    ...
    model.fit([G_train_input,P_train_obs,T_train_obs,ETpot_train_obs], G_train_output, epochs=100, batch_size=1,validation_split=0.2,callbacks=callback_list,shuffle=True)
    model=load_model(save_path, custom_objects={'HBV_1st_layer': HBV_1st_layer}) 
    
    pred_train=model.predict([G_train_input,P_train_obs,T_train_obs,ETpot_train_obs], batch_size=1)
