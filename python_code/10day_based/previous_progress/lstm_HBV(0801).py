...
class HBV_layer(Layer):
    ...
    def call(self,tensor):
        ...
        (adjust_parameter,P_obs, T_obs, ETpot_obs, G_obs_in, parPCORR,parLP,parBETA,parFC) = tf.split(tensor, num_or_size_splits=[1,1,1,1,1,1,1,1,1], axis=1)
        ...
        GW_forecast = tf.multiply(tf.add(SM,SM_),adjust_parameter)
...
def DNN_model_1(timestep):
    ...
    # output1=Dense(18, activation='linear')(output1)
    output1=Dense(1, activation='linear')(output1)
...
if __name__ == '__main__':
    ...
    fixed_parameter=(pd.read_csv(r"D:\important\research\groundwater_forecast\python_code\puyun\result\optima_parameter.csv",index_col=0)).astype(np.float32)
    timestep=3
    """ data preprocessing """ 
    ...
    # G_norm_out_module=normalization(G_train_output)
    ...
    model_train_output=G_train_output[:,timestep-1]
    """ model construction """ 
    ...
    model.fit([model_train_input_1,model_train_input_2], G_train_output[:,2], epochs=100, batch_size=32,validation_split=0.2,callbacks=callback_list,shuffle=True)
    ...
    model_test_output=G_test_output[:,timestep-1]
    ...
    train_R2=error_indicator.np_R2(model_train_output,pred_train)
    train_mape=error_indicator.np_mape(model_train_output,pred_train)
    test_R2=error_indicator.np_R2(model_test_output,pred_test)
    test_mape=error_indicator.np_mape(model_test_output,pred_test)

    pred_train_new=convert_result(pred_train,33)
    pred_test_new=convert_result(pred_test,33)
    
    
    train_new_R2=[]
    train_new_R2=error_indicator.np_R2(np.squeeze(G_multi_output[:int(len(G_multi_output)*0.8),2,:]),pred_train_new)
    train_new_mape=error_indicator.np_mape(np.squeeze(G_multi_output[:int(len(G_multi_output)*0.8),2,:]),pred_train_new)
    test_new_R2=error_indicator.np_R2(np.squeeze(G_multi_output[int(len(G_multi_output)*0.8):,2,:]),pred_test_new)
    test_new_mape=error_indicator.np_mape(np.squeeze(G_multi_output[int(len(G_multi_output)*0.8):,2,:]),pred_test_new)
