# -*- coding: utf-8 -*-
"""
Created on Sun Sep 11 23:35:00 2022
...
def DNN_model2(timestep):
    ...
    final_output = Lambda(lambda x: tf.expand_dims(x, -1))(final_output)
    final_output = LSTM(3,stateful=False,return_sequences=True,kernel_regularizer=regularizers.L2(l2=1e-3))(final_output);
    final_output = LSTM(3,stateful=False,return_sequences=False,kernel_regularizer=regularizers.L2(l2=1e-3))(final_output);    
    final_output = Dense(6, activation='linear')(final_output)
    final_output = Dense(3, activation='linear')(final_output)
...
G_obs = G_multi_input_origin[:,8,:]
G_shuffle_train_obs, G_shuffle_test_obs = G_obs[shuffle_index[:int(len(shuffle_index)*0.8)],:], G_obs[shuffle_index[int(len(shuffle_index)*0.8):],:]
G_test_obs = G_obs[int(len(shuffle_index)):,:]
...
simulate_train=[]
simulate_train.append(np.array(pd.read_excel(r"D:\important\research\groundwater_forecast\python_code\puyun\difference_level\result\1st_layer\simulate(train).xlsx",sheet_name="simulate_result(t+1)",index_col=0)))
simulate_train.append(np.array(pd.read_excel(r"D:\important\research\groundwater_forecast\python_code\puyun\difference_level\result\1st_layer\simulate(train).xlsx",sheet_name="simulate_result(t+2)",index_col=0)))
simulate_train.append(np.array(pd.read_excel(r"D:\important\research\groundwater_forecast\python_code\puyun\difference_level\result\1st_layer\simulate(train).xlsx",sheet_name="simulate_result(t+3)",index_col=0)))

simulate_test=[]
simulate_test.append(np.array(pd.read_excel(r"D:\important\research\groundwater_forecast\python_code\puyun\difference_level\result\1st_layer\simulate(test).xlsx",sheet_name="simulate_result(t+1)",index_col=0)))
simulate_test.append(np.array(pd.read_excel(r"D:\important\research\groundwater_forecast\python_code\puyun\difference_level\result\1st_layer\simulate(test).xlsx",sheet_name="simulate_result(t+2)",index_col=0)))
simulate_test.append(np.array(pd.read_excel(r"D:\important\research\groundwater_forecast\python_code\puyun\difference_level\result\1st_layer\simulate(test).xlsx",sheet_name="simulate_result(t+3)",index_col=0)))
...
simulate_merge_ = np.concatenate([simulate_merge_train,simulate_merge_test])
simulate_merge_ = simulate_merge([simulate_merge_[1:,i,:]-G_multi_input_origin[:,8,:] for i in range(0,3)])

simulate_shuffle_train = simulate_merge_[shuffle_index[:int(len(shuffle_index)*0.8)],:]
simulate_shuffle_train = np.concatenate([simulate_shuffle_train[:,:,i] for i in range(0,len(simulate_shuffle_train[0,0,:]))])

simulate_shuffle_test = simulate_merge_[shuffle_index[int(len(shuffle_index)*0.8):],:,:]
simulate_shuffle_test = np.concatenate([simulate_shuffle_test[:,:,i] for i in range(0,len(simulate_shuffle_test[0,0,:]))])

simulate_test = simulate_merge_[int(len(shuffle_index)*0.8):,:]
...
save_path=r"D:\important\research\groundwater_forecast\python_code\puyun\difference_level\model\1st_layer\hbv-lstm(shuffle).hdf5"
...
pred_test = model.predict([model_test_input_1,model_test_input_2], batch_size=32)
...
train_test_index=[]
for i in range(0,len(G_train_output)):
    if i<split_index:
        train_test_index.append(0)
    else:
        train_test_index.append(1)

train_test_index=pd.DataFrame(train_test_index)
# shuffle_index=np.array(shuffle_index.iloc[:,0])
shuffle_index_=pd.concat([pd.DataFrame(shuffle_index),train_test_index],axis=1)
shuffle_index_.columns=['shuffle_index','train_index']


#%%
"""each station performance evaluation """ 
...
for i in range(0,3):
    for station in range(0,g1_station_num):
        ...
        train_new_R2[i].append(error_indicator.np_R2(np.squeeze(G_multi_output_origin[shuffle_index[:int(len(shuffle_index)*0.8)],forecast_timestep[i],station]),pred_train_new[i][:,station]))
        train_new_rmse[i].append(error_indicator.np_RMSE(np.squeeze(G_multi_output_origin[shuffle_index[:int(len(shuffle_index)*0.8)],forecast_timestep[i],station]),pred_train_new[i][:,station]))
        test_new_R2[i].append(error_indicator.np_R2(np.squeeze(G_multi_output_origin[shuffle_index[int(len(shuffle_index)*0.8):],forecast_timestep[i],station]),pred_test_new[i][:,station]))
        test_new_rmse[i].append(error_indicator.np_RMSE(np.squeeze(G_multi_output_origin[shuffle_index[int(len(shuffle_index)*0.8):],forecast_timestep[i],station]),pred_test_new[i][:,station]))


for i in range(0,3):
    writer = pd.ExcelWriter(r"D:\important\research\groundwater_forecast\python_code\puyun\difference_level\result\1st_layer\hbvlstm(shuffle)-train(t+%s).xlsx"%(i+1),engine='xlsxwriter')
    pd.DataFrame(performance.iloc[i,:]).to_excel(writer,sheet_name="all_performance")
    pd.DataFrame(sorted_train_new[i]).to_excel(writer,sheet_name="pred_sorted")
    shuffle_index_.to_excel(writer,sheet_name="shuffle_index")
...
for i in range(0,3):
    for station in range(0,g1_station_num):
        test_new_R2[i].append(error_indicator.np_R2(np.squeeze(G_multi_output_origin[int(len(G_multi_output)*0.8):,forecast_timestep[i],station]),pred_test_new[i][:,station]))
        test_new_rmse[i].append(error_indicator.np_RMSE(np.squeeze(G_multi_output_origin[int(len(G_multi_output)*0.8):,forecast_timestep[i],station]),pred_test_new[i][:,station]))
    

for i in range(0,3):
    writer = pd.ExcelWriter(r"D:\important\research\groundwater_forecast\python_code\puyun\difference_level\result\1st_layer\hbvlstm(shuffle)-test(t+%s).xlsx"%(i+1),engine='xlsxwriter')
    pd.DataFrame(performance.iloc[i,:]).to_excel(writer,sheet_name="all_performance")
    pd.DataFrame(pred_test_new[i]).to_excel(writer,sheet_name="test_eachstation")
    pd.DataFrame(test_new_rmse[i]).to_excel(writer,sheet_name="test_rmse_eachstation")                
    pd.DataFrame(test_new_R2[i]).to_excel(writer,sheet_name="test_R2_eachstation")
