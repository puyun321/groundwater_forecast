...
For example, G01_out1_ observation groundwater level, G01_out1 obervation groundwater level difference between horizon t and t-1
"""

G01_out1_ = G01_new[timestep-2+forecast_timestep[0]:-(timestep-forecast_timestep[0])]; G01_out1=G01_out1_[1:,:]-G01_out1_[0:-1,:]
G01_out2_ = G01_new[timestep-2+forecast_timestep[1]:-(timestep-forecast_timestep[1])]; G01_out2=G01_out2_[1:,:]-G01_out2_[0:-1,:]
G01_out3_ = G01_new[timestep-2+forecast_timestep[2]:]; G01_out3 =G01_out3_[1:,:]-G01_out3_[0:-1,:]
...
G_train_out1_ = G01_out1_[1:len(G_train_out1)+1,:]
G_train_out2_ = G01_out2_[1:len(G_train_out2)+1,:]
G_train_out3_ = G01_out3_[1:len(G_train_out3)+1,:]
...
G_shuffle_train_obs = G01_new_[shuffle_index[:int(len(shuffle_index)*0.8)],:]

G_shuffle_train_out1 = G01_out1[shuffle_index[:int(len(shuffle_index)*0.8)],:]
G_shuffle_train_out2 = G01_out2[shuffle_index[:int(len(shuffle_index)*0.8)],:]
G_shuffle_train_out3 = G01_out3[shuffle_index[:int(len(shuffle_index)*0.8)],:]
...
G_shuffle_test_obs = G01_new_[shuffle_index[int(len(shuffle_index)*0.8):],:]

G_shuffle_test_out1 = G01_out1[shuffle_index[int(len(shuffle_index)*0.8):],:]
G_shuffle_test_out2 = G01_out2[shuffle_index[int(len(shuffle_index)*0.8):],:]
...
G_test_out1_ = G01_out1_[len(G_train_out1)+1:,:]
G_test_out2_ = G01_out2_[len(G_train_out2)+1:,:]
G_test_out3_ = G01_out3_[len(G_train_out3)+1:,:]
...
for index in range(0,3):
    optima = pd.read_csv(r"D:\important\research\groundwater_forecast\python_code\puyun\difference_level\result\1st_layer\optima_parameter(t+%s)-difference.csv"%(index+1),index_col=0)
    location=1
...
writer = pd.ExcelWriter(r"D:\important\research\groundwater_forecast\python_code\puyun\difference_level\result\1st_layer\simulate-shuffle(train).xlsx",engine='xlsxwriter')
...
writer = pd.ExcelWriter(r"D:\important\research\groundwater_forecast\python_code\puyun\difference_level\result\1st_layer\simulate-shuffle(test).xlsx",engine='xlsxwriter')
for i,simulate_train_result in enumerate(all_simulate_train_result):
