...
if __name__ == '__main__':
    ...
    G_train_obs, G_test_obs = G_train_input[:,timestep-1],G_test_input[:,timestep-1]
    ...
    model.fit(model_train_input, model_train_output_norm, epochs=100, batch_size=16,validation_split=0.2,callbacks=callback_list,shuffle=True)
    model=load_model(save_path, custom_objects={'HBV_layer': HBV_layer}) 
    
    #%%
    """ model forecasting """ 
    pred_train= model.predict(model_train_input, batch_size=16)
    pred_train= model_train_output_module.denorm(pred_train)
    ...
    pred_test= model_train_output_module.denorm(pred_test)    
