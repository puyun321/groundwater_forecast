...
G2_out1 = G2_new[timestep-2+forecast_timestep[0]:-(timestep-forecast_timestep[0])]
G2_out2 = G2_new[timestep-2+forecast_timestep[1]:-(timestep-forecast_timestep[1])]
G2_out3 = G2_new[timestep-2+forecast_timestep[2]:]
...
P_train_obs = P_input[train_index,:]
T_train_obs = T_input[train_index,:]
ETpot_train_obs = ETpot_input[train_index,:]
G_train_obs = G2_new_[train_index,:]

G_train_out1 = G2_out1[train_index,:]
G_train_out2 = G2_out2[train_index,:]
G_train_out3 = G2_out3[train_index,:]
G_train_out = [G_train_out1, G_train_out2, G_train_out3]

P_test_obs = P_input[test_index,:]
T_test_obs = T_input[test_index,:]
ETpot_test_obs = ETpot_input[test_index,:]
G_test_obs = G2_new_[test_index,:]

G_test_out1 = G2_out1[test_index,:]
G_test_out2 = G2_out2[test_index,:]
G_test_out3 = G2_out3[test_index,:]
...
#%%
#shuffle train test    
shuffle_index = np.array(pd.read_excel(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\sorted_predict.xlsx",sheet_name="shuffle_index").iloc[:,1])
...
for j in range(0,3):
    ...
    for station in range(0,len(G_train_obs[0])):
        G_in=G_shuffle_train_obs[:,station]    
        ...
        res=differential_evolution(HBV_model,bounds,tol=1e-3, disp=True)
        print(res.fun)
        print(res.success)
        print(res.x)
...
for index,optima in enumerate(optima_alltimestep):
    ...
    optima.to_csv(r"D:\important\research\groundwater_forecast\python_code\puyun\result\2nd_layer\optima_parameter(t+%s).csv"%(index+1))
...
for index in range(0,3):
    optima = pd.read_csv(r"D:\important\research\groundwater_forecast\python_code\puyun\result\2nd_layer\optima_parameter(t+%s).csv"%(index+1),index_col=0)
    location=1
...
writer = pd.ExcelWriter(r"D:\important\research\groundwater_forecast\python_code\puyun\result\2nd_layer\simulate-shuffle(train).xlsx",engine='xlsxwriter')
for i,simulate_train_result in enumerate(all_simulate_train_result):
    ...
    pd.DataFrame(G_shuffle_train_out[i]).to_excel(writer,sheet_name="groundwater_observation(t+%s)"%(i+1))
...
for i in range(0,3):
    for j in range(0,G_shuffle_train_out[i].shape[1]):
        ...
        obs=pd.DataFrame(G_shuffle_train_out[i][:,j])
...
writer = pd.ExcelWriter(r"D:\important\research\groundwater_forecast\python_code\puyun\result\2nd_layer\simulate-shuffle(test).xlsx",engine='xlsxwriter')
for i,simulate_train_result in enumerate(all_simulate_train_result):
    pd.DataFrame(simulate_test_result).to_excel(writer,sheet_name="simulate_result(t+%s)"%(i+1))
    pd.DataFrame(G_shuffle_test_out[i]).to_excel(writer,sheet_name="groundwater_observation(t+%s)"%(i+1))
...
for i in range(0,3):
    for j in range(0,G_shuffle_test_out[i].shape[1]):
        ...
        obs=pd.DataFrame(G_shuffle_test_out[i][:,j])
