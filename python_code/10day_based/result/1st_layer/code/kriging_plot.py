...
obs_train=pd.read_excel(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\groundwater_obs_data.xlsx",sheet_name="t+1",index_col=0)
pred_train=pd.read_excel(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\sorted_predict(hbv-lstm).xlsx",sheet_name="t+1(shuffle)",index_col=0)
simulate_train=(pd.read_excel(r"D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\sorted_predict.xlsx",sheet_name="t+1(shuffle)",index_col=0)).iloc[1:,:]
...
time = G0_.index; train_time = time[9:len(obs_train)+9]
...
pred_train_z = [Ordinary_Kriging(np.squeeze(pred_train.iloc[i, :]),G01_station_info,G_grid_lon,G_grid_lat)[0] for i in range(0,len(pred_train))]
pred_train_z = np.nan_to_num(pred_train_z)

obs_train_z = [Ordinary_Kriging(np.squeeze(obs_train.iloc[i, :]),G01_station_info,G_grid_lon,G_grid_lat)[0] for i in range(0,len(obs_train))]
...
# time=0
for time in range(0,len(pred_train)):
    predictions_array = pred_train_z[time,:,:]
    obs_array = obs_train_z[time,:,:]
    filename=str(train_time[time])[0:10]
    ...
    for i in range(len(lons)):        
        ...
        plt.axis('off')  # 去除圖片外框
    plt.title('prediction time %s'%train_time[time], fontsize=20)
        # output_path=r'important\research\groundwater_forecast\python_code'
    output_path=r'D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\kriging figure\train\regional_predict\%s'%filename
    ...
    for i in range(len(lons)):        
        plt.text(lons[i]+0.05,lats[i]+0.02,station_name[i], color='k', fontsize=12)  # 將測站名稱標示在座標點上
        plt.axis('off')  # 去除圖片外框
    plt.title('obs time %s'%train_time[time], fontsize=20)
        # output_path=r'important\research\groundwater_forecast\python_code'
    output_path=r'D:\important\research\groundwater_forecast\python_code\puyun\result\1st_layer\kriging figure\train\regional_obs\%s'%filename
    plt.savefig(os.path.join(output_path+'.png'), bbox_inches='tight')  # bbox_inches是修改成窄邊框
    plt.close()
    plt.cla()
    plt.clf()
